...
class _EditProductScreenState extends ConsumerState<EditProductScreen> {
  ...
  Future<void> _loadProduct() async {
    try {
      ...
      setState(() {
        ...
        _unit = product.unit?.trim().isNotEmpty == true
            ? product.unit!
            : 'عدد';
        for (final m in product.models) {
          final row = _ModelRow(id: m.id, isNew: false);
          row.name.text = m.name;
          row.price.text = m.price != null ? '${m.price}' : '';
          row.packageType = m.packageType;
          row.unitsPerBox.text = m.unitsPerBox != null ? '${m.unitsPerBox}' : '';
          _rows.add(row);
        }
        _loading = false;
      });
  ...
  Future<void> _deleteModelRow(int index) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('مدل «${row.name.text}» حذف شود؟', style: const TextStyle(color: Colors.white70)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('انصراف', style: TextStyle(color: Colors.white54))),
    ...
    try {
      ...
      for (final row in _rows) {
        final modelData = {
          ...
          'price': row.price.text.trim().isEmpty ? null : row.price.text.trim(),
          'packageType': row.packageType,
          'unitsPerBox': row.unitsPerBox.text.trim().isEmpty ? null : row.unitsPerBox.text.trim(),
        };
        if (row.id != null) {
          await _api.updateModel(row.id!, modelData);
        } else {
          await _api.createProduct({'name': name, 'models': [modelData]});
        }
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('محصول ویرایش شد'), backgroundColor: _green),
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: _surface,
        title: const Text('ویرایش محصول', style: TextStyle(color: Colors.white)),
        leading: IconButton(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                ElevatedButton(
                  ...
                  child: _saving
                      ...
                      : const Text('ذخیره تغییرات', style: TextStyle(color: Colors.black87, fontSize: 16, fontWeight: FontWeight.w700)),
                ),
              ]),
            ),
    );

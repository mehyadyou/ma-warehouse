    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(item.subtitle, style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: 11)),
          if (item.createdAt != null) ...[
            const SizedBox(height: 4),
            Row(children: [
              Icon(Icons.schedule_rounded, size: 11, color: Colors.white.withValues(alpha: 0.35)),
              const SizedBox(width: 4),
              Text(_dateTime(item.createdAt), style: TextStyle(color: Colors.white.withValues(alpha: 0.35), fontSize: 10.5)),
            ]),
          ],
        ])),
        Container(
          ...
          child: Text(item.status, style: TextStyle(color: item.statusColor, fontSize: 11.5, fontWeight: FontWeight.w600)),
        ),
      ]),
    );
  }

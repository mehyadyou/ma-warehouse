...
/// نمودار لایه‌لایه با منطق هارد ویندوز:
  ...
  Widget build(BuildContext context) {
    ...
    return summaryAsync.when(
      loading: () => Padding(
        ...
        child: Container(
          height: 110,
          decoration: BoxDecoration(color: const Color(0xFF1A1D22), borderRadius: BorderRadius.circular(16)),
          child: const Center(child: CircularProgressIndicator(color: Color(0xFF4ADE80))),
        ),
      ),
      error: (_, __) => Padding(
        padding: const EdgeInsets.fromLTRB(20, 0, 20, 16),
      ...
      data: (summary) {
        ...
        final items = data.asMap().entries.map((entry) {
          return InventoryItem(
            ...
            count: entry.value.totalCount.toInt(),
            unit: (entry.value.unit ?? '').trim().isNotEmpty
                ? entry.value.unit!
                : 'عدد',
        ...
        final totalRegisteredProducts = summary.totalRegisteredProducts.toInt() > 0
            ? summary.totalRegisteredProducts.toInt()
            : items.length;
        final totalInventoryUnits = summary.totalInventoryUnits.toInt();

    ...
    submitCheckin: async (
        ...
        if (returnedSerials.length) {
            ...
            for (const serial of returnedSerials) {
                ...
                // هر واحدِ واقعاً خروج‌زده می‌تواند دوباره مرجوعی شود (چرخهٔ چندبارهٔ
                // «خروج → مرجوعی» برای همان سریال). جلوگیری از ثبتِ دوبارهٔ هم‌زمان با
                // قفل شرطیِ پاک‌کردن سریال داخل تراکنش انجام می‌شود، نه با بلاک entryType.
            }
        }
